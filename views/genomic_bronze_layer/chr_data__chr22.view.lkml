...
explore: chr_data__chr22 {
  hidden: yes

  join: chr_data__chr22__call {
    view_label: "Chr Data Chr22: Call"
    sql: LEFT JOIN UNNEST(${chr_data__chr22.call}) as chr_data__chr22__call ;;
    relationship: one_to_many
  }

  join: chr_data__chr22__names {
    view_label: "Chr Data Chr22: Names"
    sql: LEFT JOIN UNNEST(${chr_data__chr22.names}) as chr_data__chr22__names ;;
    relationship: one_to_many
  }

  join: chr_data__chr22__filter {
    view_label: "Chr Data Chr22: Filter"
    sql: LEFT JOIN UNNEST(${chr_data__chr22.filter}) as chr_data__chr22__filter ;;
    relationship: one_to_many
  }

  join: chr_data__chr22__call__ad {
    view_label: "Chr Data Chr22: Call Ad"
    sql: LEFT JOIN UNNEST(${chr_data__chr22__call.ad}) as chr_data__chr22__call__ad ;;
    relationship: one_to_many
  }

  join: chr_data__chr22__call__pl {
    view_label: "Chr Data Chr22: Call Pl"
    sql: LEFT JOIN UNNEST(${chr_data__chr22__call.pl}) as chr_data__chr22__call__pl ;;
    relationship: one_to_many
  }

  join: chr_data__chr22__call__sb {
    view_label: "Chr Data Chr22: Call Sb"
    sql: LEFT JOIN UNNEST(${chr_data__chr22__call.sb}) as chr_data__chr22__call__sb ;;
    relationship: one_to_many
  }

  join: chr_data__chr22__call__genotype {
    view_label: "Chr Data Chr22: Call Genotype"
    sql: LEFT JOIN UNNEST(${chr_data__chr22__call.genotype}) as chr_data__chr22__call__genotype ;;
    relationship: one_to_many
  }

  join: chr_data__chr22__alternate_bases {
    view_label: "Chr Data Chr22: Alternate Bases"
    sql: LEFT JOIN UNNEST(${chr_data__chr22.alternate_bases}) as chr_data__chr22__alternate_bases ;;
    relationship: one_to_many
  }
}

view: chr_data__chr22 {
  sql_table_name: `capsule-plateform.genomic_bronze_layer.chr_data__chr22`
    ;;
...
view: chr_data__chr22__call {
  dimension: ad {
    hidden: yes
    sql: AD ;;
  }

  dimension: chr_data__chr22__call {
    type: string
    description: "One record for each call."
    hidden: yes
    sql: chr_data__chr22__call ;;
  }
...
view: chr_data__chr22__names {
  dimension: chr_data__chr22__names {
    type: string
    description: "Variant names (e.g. RefSNP ID)."
    sql: chr_data__chr22__names ;;
  }
}

view: chr_data__chr22__filter {
  dimension: chr_data__chr22__filter {
    type: string
    description: "List of failed filters (if any) or \"PASS\" indicating the variant has passed all filters."
    sql: chr_data__chr22__filter ;;
  }
}

view: chr_data__chr22__call__ad {
  dimension: chr_data__chr22__call__ad {
    type: number
    description: "Allelic depths for the ref and alt alleles in the order listed"
    sql: chr_data__chr22__call__ad ;;
  }
}

view: chr_data__chr22__call__pl {
  dimension: chr_data__chr22__call__pl {
    type: number
    description: "Normalized, Phred-scaled likelihoods for genotypes as defined in the VCF specification"
    sql: chr_data__chr22__call__pl ;;
  }
}

view: chr_data__chr22__call__sb {
  dimension: chr_data__chr22__call__sb {
    type: number
    description: "Per-sample component statistics which comprise the Fisher's Exact Test to detect strand bias."
    sql: chr_data__chr22__call__sb ;;
  }
}

view: chr_data__chr22__call__genotype {
  dimension: chr_data__chr22__call__genotype {
    type: number
    description: "Genotype of the call. \"-1\" is used in cases where the genotype is not called."
    sql: chr_data__chr22__call__genotype ;;
  }
}

view: chr_data__chr22__alternate_bases {
  dimension: ac {
    type: number
  ...
  dimension: chr_data__chr22__alternate_bases {
    type: string
    description: "One record for each alternate base (if any)."
    hidden: yes
    sql: chr_data__chr22__alternate_bases ;;
  }
