...
explore: chr_data__chr7 {
  hidden: yes

  join: chr_data__chr7__call {
    view_label: "Chr Data Chr7: Call"
    sql: LEFT JOIN UNNEST(${chr_data__chr7.call}) as chr_data__chr7__call ;;
    relationship: one_to_many
  }

  join: chr_data__chr7__names {
    view_label: "Chr Data Chr7: Names"
    sql: LEFT JOIN UNNEST(${chr_data__chr7.names}) as chr_data__chr7__names ;;
    relationship: one_to_many
  }

  join: chr_data__chr7__filter {
    view_label: "Chr Data Chr7: Filter"
    sql: LEFT JOIN UNNEST(${chr_data__chr7.filter}) as chr_data__chr7__filter ;;
    relationship: one_to_many
  }

  join: chr_data__chr7__call__ad {
    view_label: "Chr Data Chr7: Call Ad"
    sql: LEFT JOIN UNNEST(${chr_data__chr7__call.ad}) as chr_data__chr7__call__ad ;;
    relationship: one_to_many
  }

  join: chr_data__chr7__call__pl {
    view_label: "Chr Data Chr7: Call Pl"
    sql: LEFT JOIN UNNEST(${chr_data__chr7__call.pl}) as chr_data__chr7__call__pl ;;
    relationship: one_to_many
  }

  join: chr_data__chr7__call__sb {
    view_label: "Chr Data Chr7: Call Sb"
    sql: LEFT JOIN UNNEST(${chr_data__chr7__call.sb}) as chr_data__chr7__call__sb ;;
    relationship: one_to_many
  }

  join: chr_data__chr7__call__genotype {
    view_label: "Chr Data Chr7: Call Genotype"
    sql: LEFT JOIN UNNEST(${chr_data__chr7__call.genotype}) as chr_data__chr7__call__genotype ;;
    relationship: one_to_many
  }

  join: chr_data__chr7__alternate_bases {
    view_label: "Chr Data Chr7: Alternate Bases"
    sql: LEFT JOIN UNNEST(${chr_data__chr7.alternate_bases}) as chr_data__chr7__alternate_bases ;;
    relationship: one_to_many
  }
}

view: chr_data__chr7 {
  sql_table_name: `capsule-plateform.genomic_bronze_layer.chr_data__chr7`
    ;;
...
view: chr_data__chr7__call {
  dimension: ad {
    hidden: yes
    sql: AD ;;
  }

  dimension: chr_data__chr7__call {
    type: string
    description: "One record for each call."
    hidden: yes
    sql: chr_data__chr7__call ;;
  }
...
view: chr_data__chr7__names {
  dimension: chr_data__chr7__names {
    type: string
    description: "Variant names (e.g. RefSNP ID)."
    sql: chr_data__chr7__names ;;
  }
}

view: chr_data__chr7__filter {
  dimension: chr_data__chr7__filter {
    type: string
    description: "List of failed filters (if any) or \"PASS\" indicating the variant has passed all filters."
    sql: chr_data__chr7__filter ;;
  }
}

view: chr_data__chr7__call__ad {
  dimension: chr_data__chr7__call__ad {
    type: number
    description: "Allelic depths for the ref and alt alleles in the order listed"
    sql: chr_data__chr7__call__ad ;;
  }
}

view: chr_data__chr7__call__pl {
  dimension: chr_data__chr7__call__pl {
    type: number
    description: "Normalized, Phred-scaled likelihoods for genotypes as defined in the VCF specification"
    sql: chr_data__chr7__call__pl ;;
  }
}

view: chr_data__chr7__call__sb {
  dimension: chr_data__chr7__call__sb {
    type: number
    description: "Per-sample component statistics which comprise the Fisher's Exact Test to detect strand bias."
    sql: chr_data__chr7__call__sb ;;
  }
}

view: chr_data__chr7__call__genotype {
  dimension: chr_data__chr7__call__genotype {
    type: number
    description: "Genotype of the call. \"-1\" is used in cases where the genotype is not called."
    sql: chr_data__chr7__call__genotype ;;
  }
}

view: chr_data__chr7__alternate_bases {
  dimension: ac {
    type: number
  ...
  dimension: chr_data__chr7__alternate_bases {
    type: string
    description: "One record for each alternate base (if any)."
    hidden: yes
    sql: chr_data__chr7__alternate_bases ;;
  }
