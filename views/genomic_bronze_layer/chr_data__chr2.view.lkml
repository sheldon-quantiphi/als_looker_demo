...
explore: chr_data__chr2 {
  hidden: yes

  join: chr_data__chr2__call {
    view_label: "Chr Data Chr2: Call"
    sql: LEFT JOIN UNNEST(${chr_data__chr2.call}) as chr_data__chr2__call ;;
    relationship: one_to_many
  }

  join: chr_data__chr2__names {
    view_label: "Chr Data Chr2: Names"
    sql: LEFT JOIN UNNEST(${chr_data__chr2.names}) as chr_data__chr2__names ;;
    relationship: one_to_many
  }

  join: chr_data__chr2__filter {
    view_label: "Chr Data Chr2: Filter"
    sql: LEFT JOIN UNNEST(${chr_data__chr2.filter}) as chr_data__chr2__filter ;;
    relationship: one_to_many
  }

  join: chr_data__chr2__call__ad {
    view_label: "Chr Data Chr2: Call Ad"
    sql: LEFT JOIN UNNEST(${chr_data__chr2__call.ad}) as chr_data__chr2__call__ad ;;
    relationship: one_to_many
  }

  join: chr_data__chr2__call__pl {
    view_label: "Chr Data Chr2: Call Pl"
    sql: LEFT JOIN UNNEST(${chr_data__chr2__call.pl}) as chr_data__chr2__call__pl ;;
    relationship: one_to_many
  }

  join: chr_data__chr2__call__sb {
    view_label: "Chr Data Chr2: Call Sb"
    sql: LEFT JOIN UNNEST(${chr_data__chr2__call.sb}) as chr_data__chr2__call__sb ;;
    relationship: one_to_many
  }

  join: chr_data__chr2__call__genotype {
    view_label: "Chr Data Chr2: Call Genotype"
    sql: LEFT JOIN UNNEST(${chr_data__chr2__call.genotype}) as chr_data__chr2__call__genotype ;;
    relationship: one_to_many
  }

  join: chr_data__chr2__alternate_bases {
    view_label: "Chr Data Chr2: Alternate Bases"
    sql: LEFT JOIN UNNEST(${chr_data__chr2.alternate_bases}) as chr_data__chr2__alternate_bases ;;
    relationship: one_to_many
  }
}

view: chr_data__chr2 {
  sql_table_name: `capsule-plateform.genomic_bronze_layer.chr_data__chr2`
    ;;
...
view: chr_data__chr2__call {
  dimension: ad {
    hidden: yes
    sql: AD ;;
  }

  dimension: chr_data__chr2__call {
    type: string
    description: "One record for each call."
    hidden: yes
    sql: chr_data__chr2__call ;;
  }
...
view: chr_data__chr2__names {
  dimension: chr_data__chr2__names {
    type: string
    description: "Variant names (e.g. RefSNP ID)."
    sql: chr_data__chr2__names ;;
  }
}

view: chr_data__chr2__filter {
  dimension: chr_data__chr2__filter {
    type: string
    description: "List of failed filters (if any) or \"PASS\" indicating the variant has passed all filters."
    sql: chr_data__chr2__filter ;;
  }
}

view: chr_data__chr2__call__ad {
  dimension: chr_data__chr2__call__ad {
    type: number
    description: "Allelic depths for the ref and alt alleles in the order listed"
    sql: chr_data__chr2__call__ad ;;
  }
}

view: chr_data__chr2__call__pl {
  dimension: chr_data__chr2__call__pl {
    type: number
    description: "Normalized, Phred-scaled likelihoods for genotypes as defined in the VCF specification"
    sql: chr_data__chr2__call__pl ;;
  }
}

view: chr_data__chr2__call__sb {
  dimension: chr_data__chr2__call__sb {
    type: number
    description: "Per-sample component statistics which comprise the Fisher's Exact Test to detect strand bias."
    sql: chr_data__chr2__call__sb ;;
  }
}

view: chr_data__chr2__call__genotype {
  dimension: chr_data__chr2__call__genotype {
    type: number
    description: "Genotype of the call. \"-1\" is used in cases where the genotype is not called."
    sql: chr_data__chr2__call__genotype ;;
  }
}

view: chr_data__chr2__alternate_bases {
  dimension: ac {
    type: number
  ...
  dimension: chr_data__chr2__alternate_bases {
    type: string
    description: "One record for each alternate base (if any)."
    hidden: yes
    sql: chr_data__chr2__alternate_bases ;;
  }
